...
export class DB_Createroom implements ConfHolder {
    public datas: server.server.DB_CreateroomArray | null = null;
    public pool: Map<number, server.server.DB_Createroom> = new Map<number, server.server.DB_Createroom>();
    ...
    decode(buff: Uint8Array): Error | null {
        this.datas = server.server.DB_CreateroomArray.decode(buff);
        this.arrangeData();
        return null;
    }
    ...
    private arrangeData() {
        if (this.datas != null) {
            for (let k of this.datas.Arr.keys()) {
                ...
                this.pool.set(id!, this.datas.Arr[k] as server.server.DB_Createroom)
            }
        }
    }
}
