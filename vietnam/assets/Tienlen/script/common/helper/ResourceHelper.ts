...
export default class ResourceHelper {
    ...
    static async loadResListInAB(abName: string, paths: string[][], progressCallback: ProgressCallback, completeCallback?: (err: Error | null) => void) {
        ...
        let _getType = (type: string) => {
            if (type == "spriteatlas") return SpriteAtlas;
            else if (type == "spriteframe") return SpriteFrame;
            else if (type == "audioclip") return AudioClip;
            else if (type == "buffer") return BufferAsset;
            else if (type == "text") return TextAsset;
            else if (type == "json") return JsonAsset;
            else if (type == "scene") return SceneAsset;
            return Prefab;
        }
        let _getProgress = () => {
        ...
        for (let index = 0; index < paths.length; index++) {
            ...
            progressMap[path] = 0;
            let _type: any = _getType(paths[index][1]);
