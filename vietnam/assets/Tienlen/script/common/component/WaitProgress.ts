...
export class WaitProgress extends Component {
    ...
    onLoad() {
        // this._maskGraphics = this.getComponent(Mask).graphics;
    ...
    public get maskGraphics(): Graphics {
        // this._maskGraphics = this._maskGraphics || this.getComponent(Mask).graphics;
    ...
    update(dt: number) {
        ...
        if (angle >= Math.PI * 2) {
            ...
            this._warnFlag && TienlenHelper.playSound(TienlenDefine.SoundCountdown);
