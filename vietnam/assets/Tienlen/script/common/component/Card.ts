...
import { CardColor, CardPoint, TienlenCardColor, TienlenCardPoint } from '../define';
...
export class Card extends Component {
    ...
    public async flip(id?: number) {
        await TienlenHelper.playAnim(this._anim, Card.ANI_FLIP);
        if (id != undefined)
            this.cardId = id;
    }

    public async backFlip(id?: number) {
        await TienlenHelper.playAnim(this._anim, Card.ANI_BACKFLIP);
        if (id != undefined)
            this.cardId = id;
    }
    ...
    public set cardId(id: number) {
        ...
        let color = CardHelper.getTienlenCardColor(id);
        if (this.isJoker) {
            this.updateSpKing(color);
        } else {
            let point = CardHelper.getTienlenCardPoint(id);
