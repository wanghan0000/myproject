...
export default class NBSlotsMachine extends Component implements NBSlotsMachineIF {
    ...
    onLoad() {
        ...
        this.slotsSlots.forEach((v, column) => {
            v.rollHook = {
                ...
                initEnd: () => {
                    this.initNum += 1;
                    if (this.initNum >= this.slotsSlots.length) {
                        this.slotsMachineHook?.initEnd?.();
                    }
                }
            }
        })
    ...
    public initMacgineEnd(): boolean {
        return this.initNum >= this.slotsSlots.length
    }
    ...
    public getWindowChildrens(): Node[] {
        ...
        this.slotsSlots.forEach((v, index) => {
            ...
            v.childrens.forEach((n: any, i) => {
                if (v.blurInScreenByItem(n, 0.95)) {
                    ret.push(n);
                }
    ...
    stopMachine(params: StopParams[]): number[] {
        ...
        this.slotsSlots.forEach((v, index) => {
            if (params[index]) {
                const { fastRoll, fastRollDelay, timeInterval } = params[index];
                if (v.elapsed > v.duration) {
                    v.duration = v.elapsed + timeInterval;
                }
                if (fastRollDelay) {
                    v.duration = v.duration + fastRollDelay;
                }

                if (index != 0 && !params[index - 1].fastRoll) {
                    v.duration = this.slotsSlots[index - 1].duration + timeInterval;
                }
