import { _decorator, Component, Node, Sprite, Label, EditBox, Tween, tween } from 'cc';
...
@ccclass('UIBagItemSend')
export class UIBagItemSend extends UIScreen {
    start() {
    ...
    initView(){
        ...
        //名字
        //contentnode.getChildByName("itemname").getComponent(Label).string = this.data.Name;
        //数量
        //contentnode.getChildByName("cnttxt").getComponent(Label).string = this.data.ItemNum.toString();
...
onBtnFriendClick(){
    AudioMgr.getInstance().playSound(LobbyAudioCfg.click);
    UIMgr.getInstance().showLobbyBagFriend();
}


    //获取到一个还有回调
    onGetTargetFriend(snId:number){
        this.node.getChildByName("sendnode").getChildByName("EditBoxID").getComponent(EditBox).string = snId.toString();
    }
    ...
    onBtnOK(){
        AudioMgr.getInstance().playSound(LobbyAudioCfg.click);
        if(this.curCnt<=0){
            UIMgr.getInstance().AlertTxt(I18nMgr.getInstance().getStringByKey("lobby.xuanzezhegnqueshuliangTxt"));
            return;
        }
        let snId = this.node.getChildByName("sendnode").getChildByName("EditBoxID").getComponent(EditBox).string;
        let snIdInt = 0;
        if(Utils.chkStrIsInt(snId)){
            snIdInt = parseInt(snId);
        }else{
            //UIMgr.getInstance().AlertTxt("请输入正确的被赠送者");
            UIMgr.getInstance().AlertTxt(I18nMgr.getInstance().getStringByKey("lobby.enterrightsnid"));
            return;
        }
        let pack = new bag.bag.CSUpBagInfo();
        pack.ItemId = this.data.ItemId;
        pack.ItemNum = this.curCnt;
        pack.AcceptSnId = snIdInt;
    ...
    //发送道具的消息回调
    onSendCallBack(result:bag.bag.SCUpBagInfo){
        if(result.RetCode == bag.bag.OpResultCode.OPRC_Sucess){
            ...
            UIMgr.getInstance().AlertTxt(I18nMgr.getInstance().getStringByKey("lobby.zengsongchenggongTxt"));
        }else if(result.RetCode == bag.bag.OpResultCode.OPRC_DbErr){
            UIMgr.getInstance().AlertTxt(I18nMgr.getInstance().getStringByKey("lobby.zengsongshibaiTxt"));
            ...
            UIMgr.getInstance().AlertTxt(I18nMgr.getInstance().getStringByKey("lobby.recplayerbucunzaiTxt"));
        }
        this.updateItemInfo();
    }
