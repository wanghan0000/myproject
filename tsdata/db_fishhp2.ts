...
export class DB_FishHP2 implements ConfHolder {
    public datas:server.DB_FishHP2Array | null = null;
    public pool:Map<number,server.DB_FishHP2> = new Map<number,server.DB_FishHP2>();
    ...
    decode(buff: Uint8Array): Error|null {
        this.datas = server.DB_FishHP2Array.decode(buff);
        this.arrangeData();
        return null;
    }
    ...
    private arrangeData(){
        if (this.datas != null){
            for (let k of this.datas.Arr.keys()){
                ...
                this.pool.set(id!,this.datas.Arr[k] as server.DB_FishHP2)
            }
        }
    }
}
