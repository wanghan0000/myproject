...
export class DB_Fish implements ConfHolder {
    public datas:server.DB_FishArray | null = null;
    public pool:Map<number,server.DB_Fish> = new Map<number,server.DB_Fish>();
    ...
    decode(buff: Uint8Array): Error|null {
        this.datas = server.DB_FishArray.decode(buff);
        this.arrangeData();
        return null;
    }
    ...
    private arrangeData(){
        if (this.datas != null){
            for (let k of this.datas.Arr.keys()){
                ...
                this.pool.set(id!,this.datas.Arr[k] as server.DB_Fish)
            }
        }
    }
}
